...
import type { FormModel as OldFormModel } from "../version-37.5.0/FormModel.js";
...
export default function (model: OldFormModel): FormModel {
	...
	if (annotation?.value) {
		(annotation as { value: string }).value = transformBindingConfigHeights(annotation.value);
	}

	return model as FormModel;
}

function transformBindingConfigHeights(bindingConfig: string): string {
	...
	for (const entry of entries) {
		...
		for (const component of entry.details.components ?? []) {
			...
			if (props === undefined || !("height" in props)) {
				continue;
			}
			const { height } = props;
			if (typeof height === "number") {
				...
				throw new Error(
					`Unexpected type "${typeof height}" for "height" in bindingConfiguration component "${component.name ?? "(unknown)"}". Expected number or string.`
				);
			}
		}
	}
