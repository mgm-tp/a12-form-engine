...
} from "../../../../../models/internal/utils/EditableElement.js";
 ...
 * @ignore
 */
export namespace CorrectionModeUtil {
	/**
	 * @internal
	 *
	...
	function createCorrectionModeItem(
		...
		let repeatStaticState: ReadonlyObjectMap<EngineStore.Repeat.StaticState> | undefined =
			undefined;
		...
		if (
			repeat !== undefined &&
			(FormModel.FieldOverviewColumn.isInstance(formModelElement) ||
				FormModel.EmbeddedRepeat.isInstance(repeat?.element))
		) {
				...
				? {
						sortingState: {
							...
							sorting: col.preferredSorting
								? (col.preferredSorting.toLowerCase() as UiStateSelectors.SortingOrder)
								: "asc"
						}
					}
				: {};
	...
	function getGroupRepetitionDocumentPath(
		...
		const referenceFieldParentPathSlice = referenceFieldPath.slice(
			0,
			referenceFieldPath.length - 1
		);
	...
	function getGroupPathFromGroupRepetitionPath(
		...
		return [
			...
			{
				elementName:
					groupRepetitionDocumentPath[groupRepetitionDocumentPath.length - 1].elementName,
				index: 0
			}
		];
	}
	...
	function createScreenStateForInlineAndEmbeddedRepeat(options: {
		...
		const repeatDocumentPath = DocumentPath.allIndicesPath(groupDocumentPath);
		...
		const expandedRowPath = FormModel.EmbeddedRepeat.isInstance(parentRepeat.element)
			? groupRepetitionDocumentPath
			: undefined;
