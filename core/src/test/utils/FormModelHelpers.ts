		...
		content: {
			...
			footerBox: {
				...
				majorButtons: config.footer?.major
					...
					: undefined,
				minorButtons: config.footer?.minor ? { button: config.footer.minor.map(button) } : undefined
			},
...
export function createFormModelContent(screenName = "screen1"): FormModel.Content {
	return {
		screens: [{ name: screenName, screenElements: [], id: screenName }],
		subHeaderBox: { id: "subHeaderBox" },
