...
export namespace MiddlewareHelpers {
	/**
	 * A middleware to spy on dispatched actions.
	 */
	export function createMiddlewareSpy(ignore: ActionCreator<any>[] = []): {
	...
	export function stubMiddleware<K extends keyof typeof Middlewares>(
		obj: typeof Middlewares,
		method: K
	): void {
		const stubbedMiddleware: Middleware = api => next => action => {
			return next(action);
		};

		beforeEach(() => {
			mock.method(obj, method, () => stubbedMiddleware as any);
		});
	}
	 ...
	 */
	export function createMiddlewareWaitForAction(waitFor: Action): {
		readonly ready: Promise<void>;
		readonly middleware: Middleware;
	} {
		let dispatcher: (next: Dispatch) => (action: Action) => Action = next => next;
	...
	export function assertActions(
		middlewareSpy: Mock<(a: AnyAction) => AnyAction>,
	 ...
	 */
	export function assertNoAction(
		middlewareSpy: Mock<(a: AnyAction) => AnyAction>,
	...
	export function assertNumberOfActions(
		middlewareSpy: Mock<(a: AnyAction) => AnyAction>,
		expectedActions: ReadonlyArray<Action | undefined>,
		actualActionsIncludeInitialAction = true
	): void {
