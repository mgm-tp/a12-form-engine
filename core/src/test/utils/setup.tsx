...
import { createStore as createClientStore } from "./client-helpers.js";
...
export function loadData(group: string, name: string, documentModel: DocumentModel): GroupInstance {
...
export function setupConnectedFormEngineWithRtl(options: ConnectedRtlOptions): ConnectedRtlWrapper {
	const store =
...
function WithClientStore(props: PropsWithChildren) {
	return <Provider store={createClientStore().store}>{props.children}</Provider>;
}

function withClientStore(
	Inner?: React.FunctionComponent<PropsWithChildren>
...
export function setupFormEngineRendererWithRtl(options: SetupWithRtlOptions): RtlRenderWrapper {
	return setupComponentWithRtl(FormEngineRenderer, {
		...options,
		AdditionalWrapper: withClientStore(options.AdditionalWrapper)
	});
}

export function setupContentBoxRendererWithRtl(options: SetupWithRtlOptions): RtlRenderWrapper {
	return setupComponentWithRtl(FormEngineContentBoxRenderer, {
		...options,
		AdditionalWrapper: withClientStore(options.AdditionalWrapper)
	});
}
...
function setupComponentWithRtl(
	...
	const component = (
		<Component
			state={state}
			config={createConfig(partialConfig)}
			eventHandlers={options.dispatchConfig ?? defaultMapDispatchToProps(mock.fn()).eventHandlers}
