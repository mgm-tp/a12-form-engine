...
describe("api.back-end.store.middleware", () => {
	describe("onLeaveRowMiddlewareFactory", () => {
		...
		const rowPath = createDocumentPath([DOCUMENT_MODEL.rootGroup], [DOCUMENT_MODEL.repeatGroup, 1]);

		describe("handles Events.Repeat.leaveRepeatRow", () => {
			...
			function executeTest(options: {
				...
				describe("if a new row is left", () => {
					it("dispatches Commands.changeRepeatInstanceStateEntry for the parent repeat", () => {
						...
						const action = Events.Repeat.leaveRepeatRow({
							repeatFormModelPath,
							rowPath: rowPath
						});
						...
						const setupOptions = {
							models,
							repeatFormModelPath,
							newRow: { rowPath: rowPath, rowState: "workingOn" }
						} as const;
						setupStore(setupOptions).dispatch(action);
						const expectedCommands = [createChangeRepeatInstanceStateEntryCommand(setupOptions)];
					...
					if (options.modelName !== "detached") {
						describe("if the new row is not on the current page", () => {
							it("dispatches Commands.changeRepeatInstanceStateEntry for the parent screen with an entry for the new row with the correct page", () => {
								...
								const setupOptions = {
									...
									repeatFormModelPath,
									data: doc,
									newRow: { rowPath: newRowPath, rowState: "workingOn" },
									sortingState: {
										sorting: "asc",
										orderPath
									}
						...
						describe("if 'disableRepeatValidationOnLeaving' is set to false in the middleware options", () => {
							it("executes a partial validation of the visible fields inside the left row", () => {
								const expectedCommand = executePartialValidationTest(true, false);
								MiddlewareHelpers.assertAction(middlewareSpy.spy, expectedCommand);
							});

							it("does not execute a partial validation if a field inside the row is blurred", () => {
								const { repeatFormModelPath } = options;
								const action = Events.valueChange({
									path: createDocumentPath(
										[DOCUMENT_MODEL.rootGroup],
										[DOCUMENT_MODEL.repeatGroup, 1],
										[DOCUMENT_MODEL.indexField]
									),
									value: "test",
									formModelElementPath: []
								});
								const doc = createDocumentForRepeatValidation([{ indexField: "row-1" }]);
								...
								const command = Commands.setMessageState({ messages: {} });
						...
						describe("if 'disableRepeatValidationOnLeaving' is set to true in the middleware options", () => {
							it("does not execute a partial validation of the visible fields inside the left row", () => {
				...
				describe("if no new row is left", () => {
					describe("and no new row is in the state", () => {
						it("does not dispatch Commands.changeRepeatInstanceStateEntry for the parent repeat", () => {
							const { repeatFormModelPath } = options;
							const action = Events.Repeat.leaveRepeatRow({
								repeatFormModelPath,
								rowPath: rowPath
							...
							const setupOptions = { models, repeatFormModelPath } as const;
							setupStore(setupOptions).dispatch(action);
							const expectedCommand = createChangeRepeatInstanceStateEntryCommand(setupOptions);
							MiddlewareHelpers.assertNoAction(middlewareSpy.spy, expectedCommand);
						});
					...
					describe("and a new row is in the state", () => {
						it("does not dispatch Commands.changeRepeatInstanceStateEntry for the parent repeat", () => {
							const { repeatFormModelPath } = options;
							const action = Events.Repeat.leaveRepeatRow({
								repeatFormModelPath,
							...
							const indexFieldDocumentPath = createDocumentPath(
								[DOCUMENT_MODEL.rootGroup],
								[DOCUMENT_MODEL.repeatGroup, 1],
								[DOCUMENT_MODEL.indexField]
							);
							...
							const setupOptions = {
								models,
								repeatFormModelPath,
								newRow: { rowPath: indexFieldDocumentPath, rowState: "workingOn" }
							} as const;
							setupStore(setupOptions).dispatch(action);
							const expectedCommand = createChangeRepeatInstanceStateEntryCommand(setupOptions);
							MiddlewareHelpers.assertNoAction(middlewareSpy.spy, expectedCommand);
				...
				function executePartialValidationTest(
					...
					const store = setupStore({
						models,
						repeatFormModelPath,
						newRow: newRow ? { rowPath: rowPath, rowState: "workingOn" } : undefined,
						data: doc,
						mwo: disablePartialValidation ? { disableRepeatValidationOnLeaving: true } : undefined
					});

					store.dispatch(action);
					const messages: ReadonlyObjectMap<EngineStore.Validation.Entry> = { ...message(1) };
			...
			function createChangeRepeatInstanceStateEntryCommand(
				options: StoreOptions & Partial<EngineStore.Repeat.Entry>
			) {
