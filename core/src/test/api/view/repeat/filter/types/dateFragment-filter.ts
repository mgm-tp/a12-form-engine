...
export function describeTestForDateFragmentFilter(models: Models, timeZoneModels: Models): void {
	...
	const COLUMN_NAME = "fieldbasedrepeatoverviewcolumn-83fe7";
	const COLUMN_NAME_TIMEZONE = "fieldbasedrepeatoverviewcolumn-d3cf8";
		...
		describe("with timezone === 'UTC'", () => {
			it("changes the filter value when the input is not empty", async () => {
				await changeFilterValueTest(
					models,
					COLUMN_NAME,
					textlineRange.triggerChange("to", "1990"),
					{ from: null, to: { data: new Date("1990-01-01T00:00:00.000Z") } }
				);
			});
		});
		...
		describe("with timezone === 'Europe/Berlin'", () => {
			it("changes the filter value when the input is not empty", async () => {
				await changeFilterValueTest(
					...
					textlineRangeTZ.triggerChange("to", "1990"),
					{ from: null, to: { data: new Date("1989-12-31T23:00:00.000Z") } },
					undefined,
					true
				);
			});
		...
		describe("with timezone === 'UTC'", () => {
			it("changes the filter value when the input is not empty", async () => {
				await changeFilterValueTest(
					models,
					COLUMN_NAME,
					textlineRange.triggerChange("from", "1990"),
					{ from: { data: new Date("1990-01-01T00:00:00.000Z") }, to: null }
				);
			});
		});
		...
		describe("with timezone === 'Europe/Berlin'", () => {
			it("changes the filter value when the input is not empty", async () => {
				await changeFilterValueTest(
					timeZoneModels,
					COLUMN_NAME_TIMEZONE,
					textlineRangeTZ.triggerChange("from", "1990"),
					{ from: { data: new Date("1989-12-31T23:00:00.000Z") }, to: null },
					undefined,
					true
				);
			});
	...
	describe("shows the right rows", () => {
		...
		const rows: RepeatRow[] = setupArrayFixture(() => {
			const createRowForDateFragment = (rowIndex: number, value: Date | null) => {
				return createRow(COLUMN_PATH, "L1_DateFragment", rowIndex, value);
			};

			return [
				createRowForDateFragment(0, new Date("2019-01-01T00:00:00.000Z")),
				createRowForDateFragment(1, new Date("2018-01-01T00:00:00.000Z")),
				createRowForDateFragment(2, new Date("2020-01-01T00:00:00.000Z")),
				createRowForDateFragment(3, null),
				createRowForDateFragment(4, new Date("2021-01-01T00:00:00.000Z"))
			];
		});
			...
			deepStrictEqual(
				filterRows(
					rows,
					{
						[COLUMN_NAME]: {
							columnPath: COLUMN_PATH,
							filter: { to: { data: new Date("2019-01-01T00:00:00.000Z") }, from: null }
						}
					},
					models.formModel,
					models.documentModel
			...
			deepStrictEqual(
				filterRows(
					rows,
					{
						[COLUMN_NAME]: {
							columnPath: COLUMN_PATH,
							filter: { to: null, from: { data: new Date("2019-01-01T00:00:00.000Z") } }
						}
					},
					models.formModel,
					models.documentModel
				...
				filterRows(
					rows,
					{
						[COLUMN_NAME]: {
							columnPath: COLUMN_PATH,
							filter: {
								to: { data: new Date("2020-01-01T00:00:00.000Z") },
								from: { data: new Date("2019-01-01T00:00:00.000Z") }
							}
						}
					},
					models.formModel,
				...
				filterRows(
					rows,
					{
						[COLUMN_NAME]: {
							columnPath: COLUMN_PATH,
							filter: {
								to: { data: new Date("2019-01-01T00:00:00.000Z") },
