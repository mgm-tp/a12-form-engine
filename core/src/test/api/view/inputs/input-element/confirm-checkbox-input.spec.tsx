...
import type { BooleanOrConfirm } from "../../../../../view/internal/components/form-engine/cells/controls/boolean/types.js";
...
describe("api.view.inputs", () => {
	describe("ConfirmInput", () => {
		...
		const baseProps = {
			...
			component: "Checkbox",
			renderFunction: (props: Inputs.InputProps<BooleanOrConfirm>) => (
				<CheckboxInput {...props} coalescing />
			),
		...
		describe("onChange", () => {
			...
			async function setup(value?: {
				...
				const props: Inputs.InputProps<DocumentModel.ConfirmType> = createProps({
					...baseProps,
					models: models,
					path: path,
					modelElement: {},
			...
			it("calls onValueChange with the changed data (true -> null)", async () => {
			...
			it("calls onValueChange with the changed data (null -> true)", async () => {
				const { wrapper, dispatchConfig } = await setup({ data: null, ui: "" });
				const input = query(wrapper.widgetMap.Checkbox).props();
