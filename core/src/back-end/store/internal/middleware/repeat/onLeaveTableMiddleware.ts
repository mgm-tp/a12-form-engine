 ...
 * @internal
 *
 * Listens to Events.Repeat.leaveTable
 */
export function onLeaveTableMiddlewareFactory(options: MiddlewareOptions): Middleware {
	return api => next => action => {
		const result = next(action);
		if (Events.Repeat.leaveRepeatTable.match(action)) {
			...
			if (
				...
				const relevantElements = collectRelevantFields(state, initialFormModelPath);
