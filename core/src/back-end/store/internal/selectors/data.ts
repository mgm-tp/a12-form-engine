...
/**@internal */
export const AttachmentDataSelectors = {
	currentFiles(
	...
	attachmentState(engineState: EngineState): EngineStore.AttachmentState | undefined {
		return engineState.data.attachmentState;
	},

	thumbnail(attachment: Attachment): Selector<string | undefined> {
		return engineState =>
			AttachmentDataSelectors.attachmentState(engineState)?.thumbnails?.[
				attachment.attachment_id ?? ""
			];
	},

	unassignedIds(engineState: EngineState): string[] | undefined {
		return AttachmentDataSelectors.attachmentState(engineState)?.unassigned;
	},

	isUnassigned(engineState: EngineState, attachment: Attachment): boolean {
		if (!attachment.attachment_id) {
			return false;
		}

		return (
			AttachmentDataSelectors.attachmentState(engineState)?.unassigned?.includes(
				attachment.attachment_id
			) ?? false
		);
	},

	isLoading(engineState: EngineState, attachmentModelPath: ModelPath): boolean {
		const { loading } = AttachmentDataSelectors.attachmentState(engineState) ?? {};
