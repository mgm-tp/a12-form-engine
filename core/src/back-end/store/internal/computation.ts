...
import type { GeneratedCodeRtConfig, GroupInstance } from "@com.mgmtp.a12.kernel/kernel-md-facade";
...
export function computeAndEvaluateDependencies(options: {
	...
	const { document, models, kernelOptions, changes } = options;
...
export function internalComputeThenValidate(options: {
	...
	const {
		...
		kernelOptions: options.middlewareOptions.kernelOptionsProvider?.(options.state)
		// see A12E-3623 for why existing changes are not passed here
	});
