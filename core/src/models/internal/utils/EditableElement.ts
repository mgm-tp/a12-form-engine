...
class EditableFieldsCollector implements ModelVisitor {
	...
	private isFieldOverviewColumnOfInlineRepeat(
		...
		return (
			FormModel.FieldOverviewColumn.isInstance(repeatColumn) &&
			FormModel.InlineRepeat.isInstance(this.elementStack[this.elementStack.length - 2])
		);
	}
	...
	visitControl(control: FormModel.Control): VisitProcess {
		...
		return stopTraversal ? VisitProcess.Stop : VisitProcess.ContinueTraversal;
	}
