...
export function computeGranularity(
	...
	while (path.length > 0) {
		const element = findByPath(documentModel, path);
...
export function isRepeatableGroup(documentModel: DocumentModel, modelPath: ModelPath): boolean {
	const element = findByPath(documentModel, modelPath);
...
function isMultiSelectDataObject(element: unknown): element is MultiSelectData[number] {
	if (!isRecord(element)) {
		return false;
	}
...
export function isMultiSelectData(value: unknown): value is MultiSelectData {
	return Array.isArray(value) && value.every(isMultiSelectDataObject);
}
