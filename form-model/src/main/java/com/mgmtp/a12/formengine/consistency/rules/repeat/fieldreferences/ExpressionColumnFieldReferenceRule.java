 ...
 */
package com.mgmtp.a12.formengine.consistency.rules.repeat.fieldreferences;
...
/**
 * Repeat expression columns must reference fields which are accessible from the bound group of the repeat.
 */
public class ExpressionColumnFieldReferenceRule extends AbstractRepeatRule {
	...
	public AbstractRepeatChecker getRuleChecker(final FormModel model, final List<Problem> problems, final DocumentModelAccess documentModelAccess) {
		...
		return new ExpressionColumnReferenceChecker(model, documentModelAccess);
	}
}
