...
/**
 * Provides consistency checks during model traversal.
 */
class ElementReferenceVisitor extends ModelVisitor {

	private static final Logger LOG = LoggerFactory.getLogger(ElementReferenceVisitor.class);
	...
	ElementReferenceVisitor(final String modelId, final DocumentModelAccess documentModelService) {
		...
		if (!(
			...
			problems.add(
				new ConsistencyProblem(
					modelId,
					FormModelCategory.MISSING_DM_ELEMENT_IN_CONTROL,
			...
			if (repeatOverviewColumn instanceof FieldBasedRepeatOverviewColumnType) {
				...
				if (!(
					...
					problems.add(new ConsistencyProblem(
						modelId,
						FormModelCategory.MISSING_DM_ELEMENT_IN_OVERVIEW_COLUMN,
