...
import static com.mgmtp.a12.formengine.consistency.DocumentModelHelper.isMetadataField;
...
public class MetadataReadonlyRule implements ConsistencyRule {
	...
	public List<Problem> execute(final FormModel model, final DocumentModelAccess documentModelAccess) {
		...
		for (final FieldConfigurationEntryType field : fieldConfiguration.getField()) {
			...
			final Optional<IElement> element = documentModelAccess.findElementById(elementRef);
