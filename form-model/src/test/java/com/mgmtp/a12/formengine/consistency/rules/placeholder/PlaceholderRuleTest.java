...
public class PlaceholderRuleTest {
	...
	public void verifyPlaceholderRuleValid() {
		final FormModel productForm = ModelLoader.loadModel(
                "com/mgmtp/a12/formengine/consistency/rules/placeholder/FormModelPlaceholderValid.json");
		final FileBasedDocumentModelResolver modelResolver = new FileBasedDocumentModelResolver("com/mgmtp/a12/formengine/consistency/rules/placeholder/");
		final List<Problem> problems = placeholderRule.execute(productForm, RuleTestHelper.createDocumentModelAccess(modelResolver, productForm));
	...
	public void verifyPlaceholderRuleInvalid() {
		final FormModel productForm = ModelLoader.loadModel(
                "com/mgmtp/a12/formengine/consistency/rules/placeholder/FormModelPlaceholderInvalid.json");
		final FileBasedDocumentModelResolver modelResolver = new FileBasedDocumentModelResolver("com/mgmtp/a12/formengine/consistency/rules/placeholder/");
		final List<Problem> problems = placeholderRule.execute(productForm, RuleTestHelper.createDocumentModelAccess(modelResolver, productForm));
