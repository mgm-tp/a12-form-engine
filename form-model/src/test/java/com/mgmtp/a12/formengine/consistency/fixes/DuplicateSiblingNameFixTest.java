 ...
 */
package com.mgmtp.a12.formengine.consistency.fixes;
...
public class DuplicateSiblingNameFixTest {
	...
	public void fixDuplicates() throws Exception {
		...
		final FormModel formModel =
			ModelLoader.loadModel("com/mgmtp/a12/formengine/consistency/fixes/DuplicateNamesModel.json");

		Assert.assertFalse(duplicateSiblingNameRule.execute(formModel, null).isEmpty());

		duplicateSiblingNameFix.fix(formModel);

		Assert.assertTrue(duplicateSiblingNameRule.execute(formModel, null).isEmpty());

		final String fixedModelString = new FormModelJsonSerializer().serialize(formModel);
