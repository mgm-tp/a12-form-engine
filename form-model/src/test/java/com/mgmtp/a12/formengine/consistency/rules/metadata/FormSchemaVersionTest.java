...
public class FormSchemaVersionTest {
	...
	public void checkWrongSchemaVersion() throws Exception {
		final String testFormModelVersion = "31.99.1";
		final FormSchemaVersionRule rule = new FormSchemaVersionRule(testFormModelVersion);
		final FormModel productForm = ModelLoader.loadModel(
			"com/mgmtp/a12/formengine/consistency/rules/metadata/ProductFormWrongVersion.json");
		final List<Problem> problems = rule.execute(productForm, null);
		assertThat(problems, hasSize(1));

		assertThat(problems.get(0).getMessage(),equalTo(String.format("Version mismatch: Form model version is 19.1.2.3 but only models with version >= %s.0.0 and <= %s are supported.", testFormModelVersion.split("\\.")[0], testFormModelVersion)));
	}

	@Test
	public void checkWrongSchemaVersionWithPreReleaseTag() throws Exception {
		final String testFormModelVersion = "31.0.0-pre.0";
		final FormSchemaVersionRule rule = new FormSchemaVersionRule(testFormModelVersion);
		final FormModel productForm = ModelLoader.loadModel(
			"com/mgmtp/a12/formengine/consistency/rules/metadata/ProductFormWrongVersionPreRelease.json");
		final List<Problem> problems = rule.execute(productForm, null);
		assertThat(problems, hasSize(1));

		assertThat(problems.get(0).getMessage(),equalTo(String.format("Version mismatch: Form model version is 30.0.0 but only models with version >= %s and <= %s are supported.", testFormModelVersion, testFormModelVersion)));
	}
}
