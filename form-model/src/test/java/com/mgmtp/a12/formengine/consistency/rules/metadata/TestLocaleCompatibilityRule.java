...
public class TestLocaleCompatibilityRule {
	...
	public void testRuleFiring(final Object[] testData) {
		...
		final FormModel mm = createFormModel(dm, mmLangs);

		final ModelResolver modelResolver = modelName -> Optional.of(dm);
		final List<Problem> problems = rule.execute(mm, RuleTestHelper.createDocumentModelAccess(modelResolver, mm));
	...
	private FormModel createFormModel(final IDocumentModel documentModel, final String... languages) {
		return new FormModel().withHeader(new Header() {
			@Override
			public String getId() {
				return "test";
			}
			...
			public String getModelType() {
				return FormModel.MODEL_TYPE;
			}

			@Override
			public String getModelVersion() {
				return FormModel.MODEL_VERSION;
			}
