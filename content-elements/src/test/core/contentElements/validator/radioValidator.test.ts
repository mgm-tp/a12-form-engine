...
import { checkValidElementForRadio } from "../../../../main/core/contentElements/modules/radio/radioValidator.js";

describe("RadioValidator", () => {
	describe("checkValidElementForRadio", () => {
		it("returns no error for a reference to a supported field", () => {
			const validTypes = ["BooleanType", "EnumerationType"] as const;

			validTypes.forEach(t => {
				const messages = checkValidElementForRadio({
					element: field(t),
					path: modelPath()
				});
		...
		it("returns an error for a reference to a group", () => {
			const messages = checkValidElementForRadio({ element: group(), path: modelPath() });
