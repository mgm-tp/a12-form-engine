...
describe("core.contentElements", () => {
	describe("DateInput", () => {
		it("renders a BufferedTextLine with the correct properties", () => {
			...
			const { componentMap } = renderWrapper(<DateInput node={mockNode} />);
		...
		describe("Picker", () => {
			describe("desktop mode", () => {
				...
				it("renders a PickerWrapper and a DatePicker when the picker button is clicked", () => {
					const mockControlSettings = getMockControlSettings();
					const mockWidgetSettings = getMockWidgetSettings();
					...
					const pickerWrapper = screen.getByDataRole(PICKER_WRAPPER);
					notStrictEqual(pickerWrapper, undefined);
				...
				it("closes the PickerWrapper when onClose is triggered", () => {
					...
					const { widgetMap, componentMap } = renderWrapper(<DateInput node={mockNode} />);
				...
				it("closes the PickerWrapper when onChange is triggered", () => {
					const mockControlSettings = getMockControlSettings();
					const mockWidgetSettings = getMockWidgetSettings();
				...
				it("calls valueChanged from the document context when onChange is triggered", () => {
					const mockControlSettings = getMockControlSettings();
					const mockWidgetSettings = getMockWidgetSettings();
				...
				it("focuses the buffered text line when onChange is triggered", () => {
					const mockControlSettings = getMockControlSettings();
					const mockWidgetSettings = getMockWidgetSettings();
		...
		describe("ValueChange on TextLine", () => {
			it("calls valueChanged from the document context when a valid value was entered", () => {
				...
				const { componentMap } = renderWrapper(
					<LocalizerContext.Provider value={getMockLocalization(() => ({ value: newValue }))}>
						<DocumentContext.Provider value={mockDocContext}>
							<DateInput node={mockNode} />
						</DocumentContext.Provider>
					</LocalizerContext.Provider>
...
function getMockWidgetSettings(options?: { readonly?: true }): BaseWidgetSettings {
	return {
		value: new Date(0),
