...
const themeStyle = import.meta.resolve(`highlight.js/styles/${THEME}.css`);

await Promise.all([
	cp(fileURLToPath(themeStyle), join(buildDir, "highlight-theme.css")),
	cp(join(srcDir, "assets"), join(buildDir, "assets"), { recursive: true })
]);
