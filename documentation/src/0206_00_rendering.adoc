...
The content of a `ControlGrid` is rendered using the `LayoutGrid` widget. The layout is applied using a lookup table. Note that the mapping can result in invalid `LayoutGrid` configurations.
